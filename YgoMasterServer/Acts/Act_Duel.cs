...
namespace YgoMaster
{
    partial class GameServer
    {
        ...
        void UpgradeLoanerDeckFinish(DuelSettings duelSettings, Player player)
        {
            ...
            playerCards.FromDictionary(player.Cards.ToDictionary());
            List<KeyValuePair<int, CardStyleRarity>> cardsMain = new List<KeyValuePair<int, CardStyleRarity>>(duelSettings.Deck[DuelSettings.PlayerIndex].MainDeckCards.GetCollection());
            List<KeyValuePair<int, CardStyleRarity>> cardsExtra = new List<KeyValuePair<int, CardStyleRarity>>(duelSettings.Deck[DuelSettings.PlayerIndex].ExtraDeckCards.GetCollection());
            List<KeyValuePair<int, CardStyleRarity>> cardsSide = new List<KeyValuePair<int, CardStyleRarity>>(duelSettings.Deck[DuelSettings.PlayerIndex].SideDeckCards.GetCollection());
            duelSettings.Deck[DuelSettings.PlayerIndex].MainDeckCards.Clear();
            duelSettings.Deck[DuelSettings.PlayerIndex].ExtraDeckCards.Clear();
            duelSettings.Deck[DuelSettings.PlayerIndex].SideDeckCards.Clear();
            foreach (KeyValuePair<int, CardStyleRarity> card in cardsMain)
            {
                CardStyleRarity style = card.Value;
                if (playerCards.GetCount(card.Key, PlayerCardKind.All, CardStyleRarity.Royal) > 0)
                {
                    if (style < CardStyleRarity.Royal)
                    {
                        style = CardStyleRarity.Royal;
                        PlayerCardKind kind = playerCards.GetCount(card.Key, PlayerCardKind.Dismantle, CardStyleRarity.Royal) > 0 ? PlayerCardKind.Dismantle : PlayerCardKind.NoDismantle;
                        playerCards.Subtract(card.Key, 1, kind, CardStyleRarity.Royal);
                    }
                }
                else if (playerCards.GetCount(card.Key, PlayerCardKind.All, CardStyleRarity.Shine) > 0)
                {
                    if (style < CardStyleRarity.Shine)
                    {
                        style = CardStyleRarity.Shine;
                        PlayerCardKind kind = playerCards.GetCount(card.Key, PlayerCardKind.Dismantle, CardStyleRarity.Shine) > 0 ? PlayerCardKind.Dismantle : PlayerCardKind.NoDismantle;
                        playerCards.Subtract(card.Key, 1, kind, CardStyleRarity.Shine);
                    }
                }
                duelSettings.Deck[DuelSettings.PlayerIndex].MainDeckCards.Add(card.Key, style);
            }
            foreach (KeyValuePair<int, CardStyleRarity> card in cardsExtra)
            {
                CardStyleRarity style = card.Value;
                if (playerCards.GetCount(card.Key, PlayerCardKind.All, CardStyleRarity.Royal) > 0)
                {
                ...
                duelSettings.Deck[DuelSettings.PlayerIndex].ExtraDeckCards.Add(card.Key, style);
            }
            foreach (KeyValuePair<int, CardStyleRarity> card in cardsSide)
            {
                CardStyleRarity style = card.Value;
                if (playerCards.GetCount(card.Key, PlayerCardKind.All, CardStyleRarity.Royal) > 0)
                {
                ...
                duelSettings.Deck[DuelSettings.PlayerIndex].SideDeckCards.Add(card.Key, style);
            }
        }
        double GetUpgradeCpuDeckMinStyleShineRate(int chapterId)
        {
            return IsMysteryDuelChapter(chapterId) ? 0.00316 : 0.00297;
        }
        double GetUpgradeCpuDeckMinStyleRoyalRate(int chapterId)
        {
            return IsMysteryDuelChapter(chapterId) ? 0.00949 : 0.00099;
        }
        ...
        {
            return IsMysteryDuelChapter(chapterId) ? 0.2 : 0.0885;
        }
        // END EDITED
        ...
        DuelSettings CreateSoloDuelSettingsInstance(Player player, int chapterId)
        {
            ...
            if (ds != null)
            {
                ...
                {
                    List<KeyValuePair<int, DuelSettings>> leCampaignChapter = SoloDuels.Where(kvp => IsLeCampaignChapter(kvp.Key)).ToList();
                    List<KeyValuePair<int, DuelSettings>> leChapters = SoloDuels.Where(kvp => IsLeChapter(kvp.Key)).ToList();
                    DuelSettings playerDuelSettings = leCampaignChapter.ElementAt(rand.Next(leCampaignChapter.Count)).Value;
                    DuelSettings cpuDuelSettings = leChapters.ElementAt(rand.Next(leChapters.Count)).Value;
                    DeckInfo[] randomDecks = {playerDuelSettings.Deck[DuelSettings.PlayerIndex], cpuDuelSettings.Deck[DuelSettings.CpuIndex]};
                ...
                if (IsLeChapter(duelSettings.chapter) || IsMysteryDuelChapter(duelSettings.chapter))
                {
                    ...
                    {
                        int randIndex = rand.Next(ItemID.Values[category].Count());
                        duelSettings.SetP2ItemValue(category, ItemID.Values[category].ElementAt(randIndex));
                        if (category == ItemID.Category.FIELD && !duel.IsMyDeck)
                        {
                            duelSettings.SetP1ItemValue(duel.IsMyDeck, category, ItemID.Values[category].ElementAt(randIndex));
                        }
                    }
                }
                PlayerCards playerCards = new PlayerCards();
                playerCards.FromDictionary(player.Cards.ToDictionary());
                List<KeyValuePair<int, CardStyleRarity>> cardsMain = new List<KeyValuePair<int, CardStyleRarity>>(duelSettings.Deck[DuelSettings.CpuIndex].MainDeckCards.GetCollection());
                List<KeyValuePair<int, CardStyleRarity>> cardsExtra = new List<KeyValuePair<int, CardStyleRarity>>(duelSettings.Deck[DuelSettings.CpuIndex].ExtraDeckCards.GetCollection());
                List<KeyValuePair<int, CardStyleRarity>> cardsSide = new List<KeyValuePair<int, CardStyleRarity>>(duelSettings.Deck[DuelSettings.CpuIndex].SideDeckCards.GetCollection());
                duelSettings.Deck[DuelSettings.CpuIndex].MainDeckCards.Clear();
                duelSettings.Deck[DuelSettings.CpuIndex].ExtraDeckCards.Clear();
                duelSettings.Deck[DuelSettings.CpuIndex].SideDeckCards.Clear();
                ...
                foreach (KeyValuePair<int, CardStyleRarity> card in cardsMain)
                {
                    ...
                    duelSettings.Deck[DuelSettings.CpuIndex].MainDeckCards.Add(cardId, style);
                }
                foreach (KeyValuePair<int, CardStyleRarity> card in cardsExtra)
                {
                    int cardId = card.Key;
                    ...
                    duelSettings.Deck[DuelSettings.CpuIndex].ExtraDeckCards.Add(cardId, style);
                }
                foreach (KeyValuePair<int, CardStyleRarity> card in cardsSide)
                {
                    int cardId = card.Key;
                    ...
                    duelSettings.Deck[DuelSettings.CpuIndex].SideDeckCards.Add(cardId, style);
                }
                // END EDITED
            }
            return duelSettings;
            ...
            if (Utils.TryGetValue(request.ActParams, "rule", out rule))
            {
                ...
                if (duelSettings != null)
                {
                    ...
                    if (!duelSettings.IsCustomDuel || string.IsNullOrEmpty(duelSettings.name[DuelSettings.PlayerIndex]))
                    {
                        ...
                        // if (duelSettings.chapter >= 1010001 && duelSettings.chapter <= 1160019 && !duel.IsMyDeck && !string.IsNullOrEmpty(duelSettings.name[DuelSettings.PlayerIndex]))
                        {
